...
module ConcreteDigraph(Vertex: COMPARABLE) = struct
  ...
  (*@ predicate edge_belongs ( g : S.t HM.t ) ( v1 : Vertex.t ) ( v2 : Vertex.t ) = 
  ...
  let empty = ()
  let create n = HM.create n
  
  let is_empty h = (HM.length h = 0)
  (*@ r = is_empty h
        ensures r <-> is_empty h.HM.dom *)

  let nb_vertex g = HM.length g
  (*@ vs = nb_vertex g 
        ensures vs = Set.cardinal g.HM.dom*)
  ...
  @ g2 = copy g1 
   ...
   let clear g = HM.clear g
   (*@ clear g
      ensures g.HM.dom = Set.empty *)

  (* let nb_edges g = HM.fold (fun _ s n -> n + S.cardinal s) g 0 *)

  let out_degree g v =
    S.cardinal
      (try HM.find g v with Not_found -> invalid_arg "[ocamlgraph] out_degree")
  (*@ d = out_degree g v
        ensures vertex_belongs g v -> d = Set.cardinal (succ g v)
        raises  Invalid_argument _ -> not vertex_belongs g v *)

  let mem_vertex g v = HM.mem g v
  (*@ b = mem_vertex g v 
        ensures b <-> Set.mem v g.HM.dom *)

  let unsafe_add_vertex g v = HM.add g v S.empty
  ...
  let add_vertex g v = if HM.mem g v then () else unsafe_add_vertex g v
  (*@ add_vertex g v  
        ensures vertex_belongs g v
...
module T = struct
  ...
  let equal = (=)
      ...
      3. Some functions were commented out for now, these functions are either higher order functions, or need something that is
